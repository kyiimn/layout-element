...
- **범위-증명 스킵 프레임은 편집 소스가 되기 전 반드시 신선화된다 (pinned + ensure)**: `ThreadEngine`의 범위-증명 스킵(`committedTail(F) < Ps` strict)은 스킵 프레임이 **구 story 참조 + 구 배치**를 함께 유지하는 것으로 성립한다 — 스킵 상태 자체는 자기모순이 없다. 그러나 (a) 스킵 프레임이 이후 **편집 소스**가 되면 writeback이 구 내용 기반으로 이뤄져 다른 프레임의 편집을 덮어쓴다 (실측: 6체인×5프레임 스트레스 후 head IME 커밋이 story 2343→2334 리버트), (b) 스킵된 상태의 렌더는 span diff 생략으로 **textarea/runMap 동기화까지 건너뛴다** — 엔진만 신선화하고 DOM 컨트롤러를 신선화하지 않으면 같은 버그가 잔존한다. 방어 3종 (모두 필수): (1) `focusParagraph`는 재포커스 포함 **항상** `ensureThreadFramesFresh(ids)`를 발화한다 (조건부 가드는 이미 포커스된 상태 재진입을 놓친다), (2) 신선화가 실제 일어났으면(`true` 반환) focused 문단을 `flushRender`/`scheduleRender`한다 — postRender의 `modelText !== textarea.value` 동기화가 runMap을 신 모델에 맞춘다, (3) `_flushThreadRelayout`은 focused 문단을 `pinned`로 전달해 배치 패스가 포커스 프레임을 절대 스킵하지 않게 한다. 검증: `scripts/verify-threading-browser.mjs` (조합 커밋 전파 + 경계 backspace), `scripts/verify-virtualization.mjs` K4 (범위-증명 prefix skip layouts=0), `scripts/verify-threading.mjs` (104).
