...
- **프레임 경계 금칙 교정은 배치 입력 인코딩(출력 변이 금지)**: 프레임 배치는 독립 실행되므로 `_applyLineBreakRules`가 프레임 경계(head 마지막 visible 라인 ↔ f2 첫 라인)를 교정하지 못한다 (R7). 교정은 **`tailClampFrom`(배치 상한)을 지정해 prev를 재배치**하는 방식이다 — 배치 결과는 항상 입력의 순수 함수이고 `charOffsets`·tail·`contentFrom`이 전부 재파생되어 체인 무중복이 구조적으로 성립한다. **배치된 출력(`columnContents`)을 사후 변이하는 교정은 금지** — `charOffsets` 평행 배열을 사후 소거하면 getCharRect/print 좌표가 x=0 폴백으로 붕괴한다. clamp가 배치를 제한하면 `_captureThreadTail`의 소진(-1) 판정은 우회하고 tail은 clamp 위치로 확정된다 (해시 키 `tc:`, 캐시 저장 포함). 워드 글자는 clamp하지 않는다 (워드 무결성 > 금칙). DOM flush는 교정된 prev(`correctedFrames`)를 소스 제외에서 제외하고 재렌더한다. **clamp 수렴은 단방향이며 cap 2회로 고정이다 (A-10)**: clamp는 감소만 한다(clamp[n+1] < clamp[n] — pull이 0이면 즉시 break, 증가 경로 없음) + head contentFrom은 불증가(追い出し 단방향 — head는 길어지지 않는다) + 시도는 cap 2회 이내. 첫 판정이 prev "이전 배치" 상태에서 출발하는 한계는 이 invariant 실측으로 봉합한다 (배치 1회 추가 비용의 조기 이동은 권장하지 않음). 검증: `scripts/verify-threading.mjs` [17] (A/B + seam + **charOffsets 파생 유지·print 폴백 방어** + **clamp 단방향 수렴 invariant**).
- **`updateOverlayContext`는 parentWidth 변화 시 컬럼 폭을 재계산한다**: `columnWidths`는 `data` setter의 `_applyColumnGapFromData`에서만 계산된다 — 경량 갱신(`updateOverlayContext`)이 `inheritStyle.parentWidth`만 바꾸면, 초기 reconcile 시간차에 parentWidth 0으로 생성된 PE의 **음수 columnWidths((0-Σgap)/N)**가 REUSE 판정(`structureUnchanged`는 갱신된 parentWidth를 비교하므로 통과)을 우회해 영구 고착된다 — 재현 증상: 1322자 문단이 315개 빈 라인(음수 폭)으로 렌더되고 verify-ime 전 항목이 붕괴한다 (bisect: childrenData 보존 가드가 치유용 wipe 재생성을 막으면서 발화). 재계산 조건은 `oldParentWidth !== newParentWidth` — 실제 폭 변화에만 O(N) 비용을 지불한다. 검증: `scripts/verify-ime.mjs` (전 시나리오), 스냅샷 byte 비교 (비-변화 경로 무영향).
...
- **스킵 프레임 참조 신선화 — 구 story 참조는 상태에서 소멸한다 (A-6, 근본 해소)**: `ThreadEngine`의 범위-증명 스킵(`committedTail(F) < Ps` strict)은 step-1에서 clean 프레임의 참조를 `refreshStoryReference(storyContent)`로 신선화한다 — setter가 세우는 `_dirty`·파생 배치 캐시 무효화를 우회하므로 step-2 스킵 판정(`hasLayoutCache`·`hasPendingChanges === false`·contentFrom 불변)이 그대로 성립하고, 스킵 프레임이 구 story 참조를 보유한 채 남는 상태(편집 소스가 되면 writeback이 구 내용으로 story를 덮어써 하류 편집 롤백 — 재현: 6체인 스트레스 후 head IME 커밋이 story 리버트)가 구조적으로 소멸한다. `refreshStoryReference`는 참조 키 없는 직접 유도 메모(`_plainTextCache`·`_styleRuns`)만 무효화한다 — `_boundaryCorrection`이 `plainText[contentFrom]`으로 금칙 판정을 하고 편집 진입 시 textarea/runMap 동기가 소비하므로, 이 2종을 남기면 참조만 교체된 프레임이 구 story plain을 공급한다. `_layoutCache`·`_dirty`는 불변(스킵 판정 전제), `_parsedContentsCache`는 참조 비교로 자가 치유된다. 검증: `scripts/verify-story-reference-refresh.mjs` (30항목 — 스킵 판정 통과/캐시·dirty 불변/해시 무영향 히트 유지/내용 변경 자가 치유/구 참조 소멸/writeback 롤백 방어). 잔여 경로 방어 3종(스킵 상태가 참조가 아닌 **DOM 렌더·textarea/runMap 동기를 건너뛴** 데서 오는 결함 방어): (1) `focusParagraph`는 재포커스 포함 **항상** `ensureThreadFramesFresh(ids)`를 발화한다 (조건부 가드는 이미 포커스된 상태 재진입을 놓친다), (2) 신선화가 실제 일어났으면(`true` 반환) focused 문단을 `flushRender`/`scheduleRender`한다 — postRender의 `modelText !== textarea.value` 동기화가 runMap을 신 모델에 맞춘다, (3) `_flushThreadRelayout`은 focused 문단을 `pinned`로 전달해 배치 패스가 포커스 프레임을 절대 스킵하지 않게 한다. 검증: `scripts/verify-threading-browser.mjs` (조합 커밋 전파 + 경계 backspace), `scripts/verify-virtualization.mjs` K4 (범위-증명 prefix skip layouts=0), `scripts/verify-threading.mjs` (104).
