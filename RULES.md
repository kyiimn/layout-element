...
- **스킵 프레임 참조 신선화 — 구 story 참조는 상태에서 소멸한다 (A-6, 근본 해소)**: `ThreadEngine`의 범위-증명 스킵(`committedTail(F) < Ps` strict)은 step-1에서 clean 프레임의 참조를 `refreshStoryReference(storyContent)`로 신선화한다 — setter가 세우는 `_dirty`·파생 배치 캐시 무효화를 우회하므로 step-2 스킵 판정(`hasLayoutCache`·`hasPendingChanges === false`·contentFrom 불변)이 그대로 성립하고, 스킵 프레임이 구 story 참조를 보유한 채 남는 상태(편집 소스가 되면 writeback이 구 내용으로 story를 덮어써 하류 편집 롤백 — 실측: 6체인 스트레스 후 head IME 커밋이 story 리버트)가 구조적으로 소멸한다. `refreshStoryReference`는 참조 키 없는 직접 유도 메모(`_plainTextCache`·`_styleRuns`)만 무효화한다 — `_boundaryCorrection`이 `plainText[contentFrom]`으로 금칙 판정을 하고 편집 진입 시 textarea/runMap 동기가 소비하므로, 이 2종을 남기면 참조만 교체된 프레임이 구 story plain을 공급한다. `_layoutCache`·`_dirty`는 불변(스킵 판정 전제), `_parsedContentsCache`는 참조 비교로 자가 치유된다. 검증: `scripts/verify-story-reference-refresh.mjs` (30항목 — 스킵 판정 통과/캐시·dirty 불변/해시 무영향 히트 유지/내용 변경 자가 치유/구 참조 소멸/writeback 롤백 방어). 잔여 경로 방어 3종(스킵 상태가 참조가 아닌 **DOM 렌더·textarea/runMap 동기를 건너뛴** 데서 오는 결함 방어): (1) `focusParagraph`는 재포커스 포함 **항상** `ensureThreadFramesFresh(ids)`를 발화한다 (조건부 가드는 이미 포커스된 상태 재진입을 놓친다), (2) 신선화가 실제 일어났으면(`true` 반환) focused 문단을 `flushRender`/`scheduleRender`한다 — postRender의 `modelText !== textarea.value` 동기화가 runMap을 신 모델에 맞춘다, (3) `_flushThreadRelayout`은 focused 문단을 `pinned`로 전달해 배치 패스가 포커스 프레임을 절대 스킵하지 않게 한다. 검증: `scripts/verify-threading-browser.mjs` (조합 커밋 전파 + 경계 backspace), `scripts/verify-virtualization.mjs` K4 (범위-증명 prefix skip layouts=0), `scripts/verify-threading.mjs` (104).
