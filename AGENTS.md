...
- **opentype.js char width**: `_charWidthMm()` uses `glyph.advanceWidth / unitsPerEm * fontSize`. `minWidthMm = spaceRatio * fontSize` floor. **cmap 미등록 한글 음절 폴백**: `_charWidthMmFromFont`는 `charToGlyphIndex()`가 0(`.notdef`)을 반환하는 한글 완성형 음절(U+AC00~U+D7A3)의 폭을 기준 글자 `가`의 advanceWidth로 대체한다 (완성형 위주 폰트의 미등록 음절 `.notdef` 반각 폭으로 인한 글자 겹침 방지. `가` 글리프가 없으면 기존 `minWidthMm` 폴백 유지). 검증: `scripts/verify-hangul-glyph-fallback.mjs`. `genCharStyle()` sets `width`/`maxWidth` in mm, inner span uses `scale: ${widthRatio * 0.88} 1` for 장평.
