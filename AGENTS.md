...
**Cursor biased position (line-boundary ownership is a property of the position value)**: The cursor is `CursorModel { offset, bias }` (`src/edit/text-edit-controller.ts`) — `bias: 'end'` means a line-end boundary offset belongs to the previous line's end (phantom end placement, `atEndOfChar: true`), `bias: 'start'` means the next line's start. A line-end boundary offset is shared by two lines (line end = next line start, same source offset); which side the cursor renders on is owned by `bias`, not by history. ArrowLeft/Right/Home/End navigation is a pure function of (biased position, engine line ranges, plainText) — do not reintroduce per-navigation state flags. The engine is the single source of line boundaries: `ParagraphEngine.cursorLineRanges` (frame-local `{startOffset, endOffset, firstVisible, lastVisible, endOfBlock}` per line) is the **only** line-boundary walk — `getLineInfoBySourceOffset`/`getLineStartSourceOffset`/`totalLineCount` delegate to the engine ranges (mapper caches the translated story-absolute ranges in `_lineRanges`; `totalLineCount` delegates to `engine.cursorLineCount`), and `visibleChars`/`maxVisibleCursorOffset` share the same `_cursorLineWalk()` — the line-boundary computation cannot structurally diverge. The mapper retains only the placement pass (leading-space skip, trailing-space atEndOfChar, phantom-end registration, hole-filling). `bias` never leaks outside the controller: textarea `setSelectionRange`, public `SelectionRange`/`CursorPosition`, `cursorMove` payload, and `caretHint` remain plain offsets (React/host API compatibility). Verification: `scripts/verify-caret-parking.mjs` (28 checks — key-sequence × caret px-position + bias corpus) must be run BEFORE any cursor-navigation change pins the current behavior.
...
npx tsx scripts/verify-chain-split.mjs # 자동 체인 분할 (group-article 감지 기사별 체인 — N체인/정책 OFF byte-identical/보수 게이트/writeback identity/print 패리티)
...
|---|---|---|
| Paragraph text rendering | `docs/TEXT_ENGINE.md` | ParagraphEngine, text wrapping, column rendering, character layout, overlap avoidance, hanging punctuation (걸침표) |
...
| Page-structure performance audit | `docs/PAGE_STRUCTURE_PERF_AUDIT.md` | MUST LOAD before any multi-page performance work or before touching `confirmThreadChain`/`ParagraphEngine.set data`/`document.layout()`/`DocumentEngine.threads`: verified defect cards (per-page thread-confirm duplication, unconditional PE cache wipe, no page dirty gating), cache inventory + invalidation conditions, InDesign/Scribus/LibreOffice cache-policy research with sources, fix roadmap (Tier 1-3) + mandatory regression list, handoff tasks (§6.6 contain 체감 검증·원복 절차, §6.7 체인 분할 — **옵션 B 자동 감지 분할 구현 완료** §6.7.5-a: `auto-thread-splitter.ts` + `_ensureAutoThreads()`, 명시적 threads 존재 시 정책 OFF) |
| Auto thread chain split | `docs/PAGE_STRUCTURE_PERF_AUDIT.md` § 6.7.5-a, `src/engine/auto-thread-splitter.ts` | MUST LOAD before touching thread data assembly, `DocumentEngine.layout()`, or Place Gun article injection semantics: grouping key contract (`contentUid` → group-article box id), story non-invention (`content: undefined` → head `textContent` fallback), conservative gates (≥2 frames, protection gate, policy OFF when explicit threads exist), materialize-once identity for writeback |
| Canvas rendering (planned) | `docs/CANVAS_RENDERING.md` | MUST LOAD before any canvas rendering work: mm 단일 소스 인벤토리 (printPostData/charOffsets/decoRects), 드로잉 명령 캐시 설계, 단계별 전환 계획 (플래그 OFF byte-identical 원칙), 하이브리드 전략 (포커스 문단 DOM 유지), 리스크 레지스터 (폰트 메트릭·IME·DPR·접근성) |
...
- **printPostData 엔진 전용 API**: 엔진 트리(`PageEngine.printPostData`)가 단일 소스. `printPostData`는 엔진 전용 API로, DOM에서 호출하지 않는다. 모든 rect/char 좌표는 **mm 단위 number**. ppm 곱셈은 외부 후처리 시스템이 수행한다.
...
- **`verticalAlign` 엔진 좌표 기반 (엔진 우선 원칙)**: 라인의 수직 정렬은 브라우저 flexbox에 위임하지 않는다. 엔진이 각 라인의 절대 y 좌표를 산출하고 DOM은 좌표에 라인을 배치한다. `genColumnStyle`은 `display: block` + `position: relative` 컨테이너이며, `genLineStyle(textBlockStyle, columnIndex, lineIndex)`이 `position: absolute` + `top: ${alignOffsetMm + lineIndex × lineHeight}mm`를 적용한다. `buildParagraphPrintPostData`, `getCharRect`, `getOffsetFromPoint` 모두 동일한 `_computeAlignOffsetMm(column, effectiveColumnHeightMm, baseFontSizeMm, columnHeightMm)` 헬퍼를 사용하여 단일 소스를 보장한다.
...
- **걸침표 (hanging punctuation)**: `ParagraphStyle.hangingPunctuation` (`boolean | { lineEnd?: boolean | 'always', lineStart? }`, 기본 `false`). ON이면 `_applyHangingPunctuation()` 패스가 금칙 패스(`_applyLineBreakRules`) 직전에 실행되어 문장부호를 컬럼 밖에 배치한다 — 행말: 닫기 부호를 위 줄 끝 파트 우측 경계에 폭의 50%만 걸치도록 배치 (반각 돌출, InDesign ぶら下げ二分 — `charOffset = partWidth - 0.5×w₀`, 이후 run은 전체 폭 스택형), 행두: 열기 부호를 아래 줄 시작 파트 좌측 밖으로 (`charOffset = -swidth`). 교정한 페어는 skip set으로 금칙 패스가 재교정하지 않는다. 걸침 글자는 `TextPartData.hangs`(`'start'|'end'`, raw content 인덱스 평행 배열)로 마킹되며 `_computeCharOffsets` 정렬 산출에서 제외된다 (visible 정렬 기준 폭도 첫 부호 안쪽 절반만큼 감소). 엣지 게이트: 마지막 파트 절대 우측 끝(`Σ left + Σ width`) === 컬럼 폭 / 첫 파트 `left === 0`일 때만 걸침. `lineEnd: 'always'`는 **강제 걸침**(InDesign ぶら下げ「強制」/ CSS `force-end`) — 페어 패스 후 per-line 패스가 들어맞은 닫기 부호도 마킹(글자 이동 없음), 블록 마지막 줄(`endOfBlock`/`endOfText`)은 제외. `lineStart` ON 시 라인 첫 글자가 열기 부호면 좌측 밖으로 마킹하는 per-line 패스(케이스 5, CSS `first` 확장)도 실행된다. `true`는 표준 걸침(InDesign「標準」/ CSS `allow-end` — 오버플로우 시에만). 걸침 ON 시 `genColumnStyle` 컬럼 overflow와 paragraph `:host` overflow가 `'visible'`로 전환된다 (OFF 시 기존 `'hidden'` 유지 — byte-identical). 검증: `scripts/verify-hanging-punctuation.mjs` (82항목), `scripts/verify-hanging-punctuation-browser.mjs` (11항목). 상세: `docs/TEXT_ENGINE.md` §23, `RULES.md` §1.9.
...
- **object-fit 엔진 우선**: `ImageEngine.displayRect`가 표시 위치/크기의 단일 소스. `cover`/`contain`/`fill`은 입력 x/y/w/h를 무시하고 `src/engine/object-fit-engine.ts`의 `computeObjectFit()`으로 자동 계산, `none`은 입력값 그대로 사용. `image.element.ts`(`_drawImage`)는 엔진의 `displayRect` 결과를 소비만 한다 — DOM은 `computeObjectFit`을 직접 호출하지 않는다.
...
- **`sourceParagraphStyle`/`sourceTextStyle` 미사용**: `_paragraphStyle`/`_textStyle` store injected values only (no merge with parent styles). `effectiveParagraphStyle`/`effectiveTextStyle` getters perform the merge: `{ ...DEFAULT, ..._inheritStyle, ..._paragraphStyle }`.
...
- **`TableCellEngine.extractData`**: Returns `TableCellData` with defaults (`colspan ?? 1`, `rowspan ?? 1`, `padding ?? 0`). `children` from `boxEngine.extractData`. No border fields — borders are managed by `TableBorderStore` on the parent `TableEngine`.
...
- **`connectedCallback`**: `genUUID()` is NOT called in any element's `connectedCallback`. ID generation is exclusively handled by `data` setter (for DOM) or engine (for Node.js standalone).
...
- **opentype.js char width**: `_charWidthMm()` uses `glyph.advanceWidth / unitsPerEm * fontSize`. `minWidthMm = spaceRatio * fontSize` floor (per-run `spaceRatio` override honored, paragraph effective fallback). **cmap 미등록 한글 음절 폴백**: `_charWidthMmFromFont`는 `charToGlyphIndex()`가 0(`.notdef`)을 반환하는 한글 완성형 음절(U+AC00~U+D7A3)의 폭을 기준 글자 `가`의 advanceWidth로 대체한다 (완성형 위주 폰트의 미등록 음절 `.notdef` 반각 폭으로 인한 글자 겹침 방지. `가` 글리프가 없으면 기존 `minWidthMm` 폴백 유지). 검증: `scripts/verify-hangul-glyph-fallback.mjs`. `genCharStyle()` sets `width`/`maxWidth` in mm (per-run `letterSpacing`/`widthRatio`/`spaceRatio`/`fontSize` overrides honored), inner span uses `scale: ${widthRatio * 0.88} 1` for 장평 (per-run `widthRatio` via `genCharInnerStyle(inlineStyle)`).
...
- **Box `_rebuildingChildren` flag**: Direct DOM `appendChild`/`remove` on a box does NOT sync the engine — the flag-based guard alone (with `_pendingData` cache) suppresses double layout/render during `data` setter reconcile. External code must use `appendChildData()`/`removeChildData()`, never raw DOM `appendChild`, or the engine tree diverges. See `docs/PERFORMANCE.md` §3.4.
