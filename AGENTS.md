...
- **걸침표 (hanging punctuation)**: `ParagraphStyle.hangingPunctuation` (`boolean | { lineEnd?, lineStart? }`, 기본 `false`). ON이면 `_applyHangingPunctuation()` 패스가 금칙 패스(`_applyLineBreakRules`) 직전에 실행되어 문장부호를 컬럼 밖에 배치한다 — 행말: 닫기 부호를 위 줄 끝 파트 우측 밖으로 (`charOffset = partWidth + Σ 선행 걸침 폭`), 행두: 열기 부호를 아래 줄 시작 파트 좌측 밖으로 (`charOffset = -swidth`). 교정한 페어는 skip set으로 금칙 패스가 재교정하지 않는다. 걸침 글자는 `TextPartData.hangs`(`'start'|'end'`, raw content 인덱스 평행 배열)로 마킹되며 `_computeCharOffsets` 정렬 산출에서 제외된다. 엣지 게이트: 마지막 파트 절대 우측 끝(`Σ left + Σ width`) === 컬럼 폭 / 첫 파트 `left === 0`일 때만 걸침. 걸침 ON 시 `genColumnStyle` 컬럼 overflow와 paragraph `:host` overflow가 `'visible'`로 전환된다 (OFF 시 기존 `'hidden'` 유지 — byte-identical). 검증: `scripts/verify-hanging-punctuation.mjs` (55항목), `scripts/verify-hanging-punctuation-browser.mjs` (11항목). 상세: `docs/TEXT_ENGINE.md` §23, `RULES.md` §1.9.
