...
- **ImageData coordinates are mm-based**: `x`/`y`/`width`/`height` in `ImageData` are **mm-based display position and size** within the box. The browser loads a proxy image (not the actual full-resolution image) — the engine computes layout coordinates for visual display only, and actual image processing (crop, print) is handled externally. `objectFit` controls how the proxy image is displayed: `cover`/`contain`/`fill` (default `cover`) compute crop region from `originalWidth`/`originalHeight`; `none` renders the entire image at the specified position/size. When `objectFit !== 'none'` and `originalWidth`/`originalHeight` are set, `_computeObjectFit` automatically computes the crop region.
