...
| `.` (물리 키 `Period`) | `Ctrl`/`Cmd`+`Shift` | 선택 영역 글자 크기 **확대** (+`shortcutSteps.fontSize`, 기본 0.1mm, per-run 상대 증감) |
| `,` (물리 키 `Comma`) | `Ctrl`/`Cmd`+`Shift` | 선택 영역 글자 크기 **축소** (−`shortcutSteps.fontSize`, 하한 0.1mm) |
| `[` (물리 키 `BracketLeft`) | `Ctrl`/`Cmd`+`Alt`+`Shift` | 선택 영역 자간 **증가** (+`shortcutSteps.letterSpacing`, 기본 0.01em, per-run 상대 증감) |
| `]` (물리 키 `BracketRight`) | `Ctrl`/`Cmd`+`Alt`+`Shift` | 선택 영역 자간 **감소** (−`shortcutSteps.letterSpacing`) |
| `[` (물리 키 `BracketLeft`) | `Ctrl`/`Cmd`+`Alt` | 선택 영역 장평 **증가** (+`shortcutSteps.widthRatio`, 기본 0.01, per-run 상대 증감) |
| `]` (물리 키 `BracketRight`) | `Ctrl`/`Cmd`+`Alt` | 선택 영역 장평 **감소** (−`shortcutSteps.widthRatio`) |
| `,` (물리 키 `Comma`) | `Ctrl`/`Cmd`+`Alt`+`Shift` | 선택 영역 공백비율 **증가** (+`shortcutSteps.spaceRatio`, 기본 0.01em, per-run 상대 증감) |
| `.` (물리 키 `Period`) | `Ctrl`/`Cmd`+`Alt`+`Shift` | 선택 영역 공백비율 **감소** (−`shortcutSteps.spaceRatio`, 하한 0) |
...
**`_getEndKeyOffset(offset)`**: End 키가 이동해야 할 실제 offset을 반환한다. 우선순위 규칙: (1) `\n` 위치나 텍스트 끝이면 그대로 반환 — 텍스트 끝에는 endOfBlock phantom placement(`atEndOfChar: true`)가 이미 등록되어 있어 이 offset 자체가 "마지막 가시 문자 오른쪽"을 의미하며, +1하면 매핑 밖 offset이 되어 line rect 폴백(라인 맨앞)으로 빠진다. (2) placement가 이미 `atEndOfChar: true`이면(trailing space로 끝나는 라인 끝) 그대로 반환 — +1하면 다음 라인 첫 글자 왼쪽으로 넘어간다. (3) 매핑된 가시 문자면 +1을 반환하여 preferLineEnd 조회로 커서가 마지막 문자의 오른쪽에 표시되도록 한다. (4) 매핑되지 않은 위치면 역방향으로 가장 가까운 매핑 위치 +1을 반환한다.
...
7. `_clearCompositionUnderline()` 호출로 조합 중 임시 장식 div 제거.
8. `_updateCursorPosition()` 호출.
...
8. `_clearCompositionUnderline()` 호출로 조합 중 임시 장식 div 제거.
...
| 상태 기반 스타일 주입 (구현됨) | 텍스트/문단 스타일 주입은 `EditManager.applyTextStyle()` 단일 진입점이 담당한다. | 판별표와 캐스케이드 동작은 § 6A.5.1 참조. 호스트는 인라인 데이터를 직접 생성하지 않는다. 정규화(`normalizeRunMap`)가 문단 기본과 동일한 런을 자동 해제하므로 content 배열은 항상 최소 런 형태를 유지한다. 정규화는 포커스/blur/주입 시 자동 수행된다. |
| 스레드 프레임 경계 이동 (구현됨) | 스레드 프레임의 편집 커서는 **story 절대 좌표계**를 쓴다 — `TextEditCoordinateMapper`가 placement/라인 맵을 `contentFrom` 기준으로 구축해 프레임 로컬(렌더 span·엔진 쿼리)과 절대(story) 공간을 변환한다. 화살표/Backspace/타이핑으로 커서가 프레임 coverage를 벗어나면 `EditManager.transferCursorToOwningThreadFrame`이 소유 프레임으로 포커스를 이관한다. 경계점은 **start와 end 양쪽**이 있다 — tail 오프셋은 이전 프레임의 visible 끝(end)이자 다음 프레임의 배치 시작(start)이므로 양쪽 coverage가 동시에 주장하며, 이동 방향이 소유를 결정한다: 좌측 이동(ArrowLeft/ArrowUp)은 start 경계점에서 이전 프레임으로, 우측/하측 이동(ArrowRight/ArrowDown)은 end 경계점에서 다음 프레임으로 편입한다. (결함 이력: end 경계점의 right 편입 분기 부재로 ArrowDown@tail이 "소유" 판정으로 early return해 다음 프레임 이관이 안 됐다 — verify-threading-browser [7] 7e/7f/7g가 잠근다.) IME 조합 중에는 이관하지 않는다(compositionend 후). 검증: `verify-threading-browser.mjs` [7]. | 비-스레딩 문단은 `contentFrom = 0`이라 변환이 항등이다 (스냅샷 byte-identical 방어). **story 절대 인덱싱 계약**: 스레드 프레임은 story 전체를 `textContent`/`plainText`로 소유하므로, 라인 walk(`ParagraphEngine._cursorLineWalk`)이나 렌더 span walk(`LayoutColumnElement.renderText`/_computePerfSourceOffsets/_computeSkippedLineOffsets)가 plain을 인덱싱하는 모든 지점(`\n` 소비 판정, 오버플로 경계의 공백/`\n` 조사)은 `plain[this._contentFrom + 로컬 offset]`처럼 **절대 인덱스로 읽어야 한다** — 워크의 누적 offset은 로컬(0 = contentFrom 글자)이고 plain은 story 절대 공간이므로. 로컬 오프셋을 그대로 인덱싱하면 2번째 프레임(contentFrom > 0)부터 `\n` 소비가 실패해 이후 라인 레인지·span key가 -1 시프트되고, 시프트가 렌더/엔진 양측에 자기일관적이라 **커서가 지정 offset보다 한 글자 오른쪽(+1)에 렌더**된다 (사용자 보고 "2번째 스레드부터 엔터 후 커서 +1" — 체인 배치 contentFrom은 파트 합산 기반이라 무영향이라 "배치는 맞는데 커서만 +1"). 검증: `verify-threading.mjs` [19] (엔진 레인지), `verify-threading-browser.mjs` [13] (절대 위치 기준 — span key 상대 비교는 시프트를 못 잡으므로 story 문자에서 기대값을 직접 산출). |
| 단일 단락 편집 | 단락을 넘어가는 선택이나 여러 단락 동시 편집은 지원하지 않는다. | `_cursorModel`과 `_selectionAnchor`가 하나의 `LayoutParagraphElement`에만 연결. 향후 문서 전역 `TextEditController`와 paragraph 간 매핑이 필요하다. 스레드 프레임 간 **selection 확장**(Shift+Arrow로 프레임을 넘는 선택)은 아직 미지원 — 커서 이관만 지원한다. |
