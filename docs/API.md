...
| `column` | `number \| number[]` (via `data`) | — | 하위 컬럼 그리드. `undefined`이면 부모 상속. `number`(단수)는 GC가 균등 분할. `number[]`(명시 너비)는 부모 box width 변경 시 비례 조정. |
| `gap` | `number \| number[]` (via `data`) | mm | 하위 컬럼 간격. 명시 설정 시 고정. 생략 시 부모 상속. |
