...
- **ppm 외부 주입 (옵셔널)**: `PageEngine` 생성 시 ppm을 파라미터로 전달. 브라우저는 100mm div 측정, Node는 PDF 엔진 설정값. **엔진 연산은 mm 단위로만 동작하므로 ppm이 없어도 정상 작동** — ppm은 브라우저 요소가 화면 렌더링을 위해 참조하는 용도.
...
- **자체 트리 관리**: 엔진 간 직접 참조로 부모-자식 트리 구성. `PageEngine.layout()` 하나로 전체 엔진 트리 자동 구축
...
| 게터 | 타입 | 설명 |
|------|------|------|
| `data` | `PageData` | 문서 데이터 |
| `extractData` | `PageData` | 엔진 현재 상태에서 조립한 문서 데이터. `children`은 자식 박스 엔진의 `extractData`에서 동적 조립. padding은 getter 기본값 적용 |
...
| 세터 | 타입 | 설명 |
|------|------|------|
| `data` | `PageData` | 데이터 갱신. `_dirty = true` (전체 교체 후에도 `layout()` 호출 필요) |
...
| `childrenData` (setter) | `BoxData[]` | 자식 박스 데이터 설정. `data` setter는 페이지 자체 속성만 담당, 자식 데이터는 이 setter가 담당. 설정 시 `_dirty = true` |
...
```ts
import { PageEngine, FontLoaderEngineImpl, ColorRegistryEngineImpl } from 'layout-element';
...
- **Node.js**: `PageEngine._buildImageEngine()`에서 base64 data URI 자동 디코딩 (pngjs 사용)
...
`PageEngine.layout()` 호출 시 `ImageEngine.rgbaData`가 자동 주입된다.
...
소비처: `computeLineHeightMm` — `ParagraphEngine._initLayoutMetrics`/`_createLineWithParts`/`_computePerLineHeights`/`_confirmLineHeight`, `GridCalculatorEngine._calcColumnGridCoords`, `PageEngine._documentContainerMetrics`. `'ratio'` 분기는 기존 `maxFontSizeMm × lineGap` 공식과 byte-identical. `resolveLineGap` — `ParagraphEngine.effectiveParagraphStyle`(병합 후 보정), `GridCalculatorEngine.lineGap` getter, `PageEngine._documentContainerMetrics`.
...
`PageEngine.layout()` 하나로 `PageData`에서 전체 엔진 트리를 자동 구축한다. DOM 요소 없이 연산 가능.

```
PageEngine (root, owns ppm + resources)
...
- `PageEngine.absRect` = `{ 0, 0, width, height }`

### 리소스 전파

- `EngineResources = { ppm, fontLoader, colorRegistry }`
- `PageEngine.resources`로 하위 엔진에 전파
...
브라우저 환경에서 `LayoutPageElement`는 싱글톤 `FontLoader`/`ColorRegistry`를 엔진 인터페이스에 맞게 변환하는 어댑터를 사용한다. 이 어댑터들은 `page.element.ts` 내부에 private으로 정의되어 외부로 노출되지 않는다.
...
|---------|--------|---------------|
| `LayoutPageElement` | `PageEngine` | `_layoutStructure()` |
...
- **Document builds the engine tree**: `LayoutPageElement._layoutStructure()`는 `PageEngine.layout(this.items.map(e => e._rawData()))`를 호출하여 자식 박스 데이터에서 전체 엔진 트리를 구축. `engine.data` setter는 자신의 속성만 설정 (children 제외), `layout()` 파라미터로 자식 데이터 전달.
- **Box attaches to tree engine**: `LayoutBoxElement._layoutStructure()`는 부모 엔진(`PageEngine`/`BoxEngine`/`TableCellEngine.boxEngine`)에서 `findBoxEngineById(this.id)`로 미리 구축된 `BoxEngine`을 찾아 연결. `id`가 없거나 트리에 없는 경우(`appendChildData`로 새로 추가된 경우)에만 새 `BoxEngine`을 생성하고 부모 `childEngines`에 등록.
...
- **Parent 구축**: `box.element.ts._findParentEngine()`이 부모 요소에서 `PageEngine`/`BoxEngine`/`TableCellEngine` 추출. 테이블 셀 내부 박스는 `TableCellEngine` 자체를 부모 엔진으로 받으며, `TableCellEngine.findBoxEngineById(id)`가 셀의 `boxEngine` 자체를 반환한다 (BoxEngine.findBoxEngineById는 자식만 검색하므로 셀 내부 박스를 찾지 못함).
...
```ts
LayoutPageElement.engine: PageEngine | undefined
...
const pageData = { width: 257, height: 370, columns: 6, gap: 3, paragraphStyle: { lineGap: 1.2 }, textStyle: { fontSize: 4, fontFamily: 'Myoungjo' }, children: [...] };
const engine = PageEngine.create(
  pageData,
  fontLoader,
  colorRegistry,
...
**값**:
`GridCalculatorEngine`, `ImageEngine`, `checkOverlapMm`, `computeOverlapSizeMm`, `engineMergeOverlapParts` (alias), `BoxEngine`, `TableEngine`, `TableRowEngine`, `TableCellEngine`, `ParagraphEngine`, `PageEngine`, `FontLoaderEngineImpl`, `ColorRegistryEngineImpl`, `computeObjectFit`, `computeLineHeightMm`, `prepareImageDecoder`, `decodeBase64ImageToRgba`, `decodeBase64ImageToRgbaSync`, `isNodeJs`, `parseDataUri`

**타입**:
`GridRect`, `AbsRect`, `EngineMmRect` (alias), `OverlapDirection`, `OverlapResult`, `OverlapInput`, `ImageEngineRef`, `BoxContentType`, `FontLoaderEngine`, `ParsedFont`, `ColorRegistryEngine`, `EngineResources`, `GridCalculatorEngineOptions`, `ImageEngineData`, `ImageLayoutResult`, `BoxLayoutResult`, `TableLayoutResult`, `ParagraphLayoutResult`, `PageLayoutResult`, `LayoutResult`, `EngineCursorPlacement` (alias), `RgbaData`, `ObjectFitRect`, `ObjectFitInput`
...
- 엔진 트리(`PageEngine` → `BoxEngine` → `ParagraphEngine` / `ImageEngine` / `TableEngine`)가 모든 레이아웃 계산의 단일 진실 공급원이다.
- DOM 요소는 엔진 결과를 표시와 WYSIWYG 편집을 위해 소비한다.
- DOM 요소는 절대 엔진 트리를 만들거나 수정하지 않는다.
- 편집이 발생하면 편집된 콘텐츠를 `PageData` / `BoxData`로 직렬화하고, 엔진이 이를 다시 처리한 뒤 결과를 DOM에 전파한다.
...
engine.data = newPageData;            // dirty = true (페이지 자체 속성)
engine.childrenData = pageData.children; // dirty = true (자식 박스 데이터)
...
// 잘못된 패턴 (예외 발생)
engine.data = newPageData;             // dirty = true
engine.printPostData;                      // ❌ throw: "PageEngine has pending..."
...
| 엔진 | 개별 setter | 커밋 메서드 |
|------|------------|-----------|
| `PageEngine` | `width`, `height`, `paddingTop/Right/Bottom/Left`, `columns`, `gap`, `paragraphStyle`, `textStyle`, `childrenData` | `layout()` |
...
- 자기(`PageEngine`) dirty 시 `layout()`, 이후 하위 트리를 재귀 순회하며 타입별 커밋 수행: Box/Image → `layout()`, Table → `layout()` + `buildCellBoxEngines()`.
