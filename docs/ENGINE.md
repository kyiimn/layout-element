...
| `editableTextHeight` | `number` | 텍스트 배치 가능 높이 (mm). padding 제외 전체 높이 (`height - paddingTop - paddingBottom`) |
...
- `editableTextHeight`는 padding 제외 전체 높이(`height - paddingTop - paddingBottom`)로, `BoxEngine.absHeight = lineHeight * height` 공식과 일치하여 N 라인 Box가 N 라인을 모두 수용하도록 보장
