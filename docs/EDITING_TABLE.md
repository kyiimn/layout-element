...
대각선(`PrintPostDiagonal`)의 끝점은 셀 경계가 아닌 **보더 안쪽 가장자리**로 보정하여 export한다. 셀의 4면 각각에 대해 `borderStore`에서 보더 두께를 조회하고, 9.4 규칙에 따라 외곽 면은 전체 두께, 내부 면은 절반 두께만큼 셀 경계에서 안쪽으로 당긴 좌표를 끝점으로 사용한다. 이렇게 하면 보더가 두꺼운 경우에도 대각선 끝점이 보더 안쪽 가장자리와 정확히 맞물려 틈이 생기지 않는다. 병합 셀(`colspan`/`rowspan > 1`)의 경우, 셀이 덮는 모든 면 세그먼트 중 최대 두께를 사용하여 보더 안쪽 가장자리가 일관되게 유지된다.
