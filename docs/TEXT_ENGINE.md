...
## 27. 스레드 체인 재배치(flush) 라이프사이클

타이핑 → 체인 재배치 → DOM 동기의 파이프라인 계약. 스레딩의 기본 메커니즘(story 단일 소스,
범위-증명 스킵, 프레임 경계 교정)은 `RULES.md §1.10`과 이 문서의 스레딩 장(§26)을
참조하고, 이 장은 **재배치 요청 → flush 실행의 스케줄링 계약**을 소유한다.
...
### 27.2 flush 재진입 계약 — "flush 중 파생 relayout은 다음 microtask로 이월된다" (E-2)
...
- coordinator(`flushThreadRelayout`)는 주입 컨텍스트(`ThreadRelayoutContext`)만
  ...
  누수되어 "요소는 엔진 소스와 EditManager만 주입" 계약(C-1)이 깨진다.
