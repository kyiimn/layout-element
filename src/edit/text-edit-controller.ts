...
export class TextEditController {
  ...
  private _cursorModel: CursorModel = { offset: 0, bias: 'start', selection: null };
   ...
   */
  get cursorBias(): CursorBias {
    return this._cursorModel.bias;
  }

  /**
   * 주차 상태 복원용 bias 설정 (paragraph.element의 disconnect/reconnect 복원 경로).
   * 커서 렌더를 다시 계산한다 — 소속 라인이 바뀔 수 있다.
   * @param bias - 복원할 소속 ('start' = 다음 라인 시작, 'end' = 이전 라인 끝)
   * @throws 없음
   */
  setCursorBias(bias: CursorBias): void {
    this._cursorModel.bias = bias;
    this._updateCursorPosition();
  }
  ...
  case "Enter": {
    ...
    case "Tab": {
        // Shift+Tab: Right Indent Tab 삽입. InDesign과 동일한 단축키.
        // 일반 Tab: 단락 간 포커스 이동(document capture handler와 동일 동작).
        event.preventDefault();
        event.stopPropagation();
        this._cursorModel.bias = 'start';
        if (!event.shiftKey) {
          this._manager.navigateByTab(false);
          break;
        }
        const tabSelection = this._cursorModel.selection;
        const { start: tStart, end: tEnd } = tabSelection?.normalized() ?? { start: null, end: null };
        const tabInsertAt = tStart?.textOffset ?? offset;
        const tabDeleteLen = (tEnd?.textOffset ?? offset) - tabInsertAt;

        model.textContent = insertTextIntoInline(
          deleteTextFromInline(model.textContent, tabInsertAt, tabDeleteLen),
          tabInsertAt,
          "\t",
        );
        this._runMap = runMapFromContent(model.textContent);
        this._textarea.value = content.slice(0, tabInsertAt) + "\t" + content.slice(tEnd?.textOffset ?? offset);
        this._cursorModel.offset = tabInsertAt + 1;
        this._textarea.setSelectionRange(tabInsertAt + 1, tabInsertAt + 1);
        this._cursorModel.selection = null;
        this._debouncedRender();
        break;
      }
  ...
  private _extendSelection(newOffset: number): void {
    ...
    // Shift 이동은 bias-blind (라인 경계 소속은 확정되지 않음 — selection이 활성인 동안 렌더가 숨는다).
    current.bias = 'start';
  }
  ...
  private _computeVerticalOffset(direction: -1 | 1): number | null {
    ...
    const fromEndParked = this._cursorModel.bias === 'end'
  ...
  private _updateCursorPosition(): void {
    ...
    if (this._cursorModel.bias === 'start' && placement?.atEndOfChar === true) {
      // bias 'start' same-line 가드: preferLineEnd가 반환한 phantom placement가
    ...
    if (this._cursorModel.bias === 'end') {
      // bias 'end': 라인 끝 주차. 기본 placement(phantom end)를 그대로 사용한다.
    ...
    if (!placement) {
      ...
      if (this._isComposing && this._compositionStartOffset > 0) {
        const startPlacement =
          ...
          this._mapper.getCursorPlacement(this._compositionStartOffset);
        if (startPlacement) {
          const startRect = this._mapper.getCharRect(startPlacement.sourceOffset);
          if (startRect) {
            const startTop = startRect.height <= 1 ? this._resolveFallbackTop(startPlacement.sourceOffset, startRect.height) : startRect.top;
            this._cursorEl.top = startTop;
            this._cursorEl.left = startPlacement.atEndOfChar ? startRect.left + startRect.width : startRect.left;
            this._cursorEl.height = startRect.height;
            this._cursorEl.visible = cursorVisible;
            this._textarea.style.top = `${clampTop(startRect.top)}px`;
            this._textarea.style.left = `${this._cursorEl.left}px`;
            return;
          }
        }
      }
...
/**
 * 커서가 라인 경계 offset 위에 있을 때 어느 쪽 라인 소속인지.
 *
 * 라인 끝과 다음 라인 시작이 같은 source offset을 공유하는 좌표계 함정에서
 * 소속을 결정한다 — `bias: 'end'`는 이전 라인 끝(phantom end placement,
 * atEndOfChar=true)으로 그려지고, `bias: 'start'`는 다음 라인 시작
 ...
 * 커서/선택의 내부 상태.
 *
 * `offset`은 story 절대 plain 오프셋 (스레드 프레임 포함 — mapper 통일 계약),
 * `bias`는 offset이 라인 경계에 있을 때 어느 쪽 소속인지를 위치 값 자체에
 * 담는다. 외부 경계(textarea setSelectionRange, SelectionRange/CursorPosition,
 * cursorMove payload, caretHint)에는 bias가 노출되지 않는다 — 이들에 bias를
 * 흘려보내면 React 레이어와 호스트 API가 깨진다.
 */
