...
| `verify-remount-canvas.mjs` | 정합성 (브라우저) | 가상화 재부착 × canvas 렌더 모드 — 재부착(unpark) 문단 canvas 기본값 유지 + 하이브리드 계약(포커스→즉시 DOM 복귀, 타이핑 커밋, blur→canvas 복귀) + 커서 저장·복원. 배경 결함: 포커스 전환이 render() 게이트 입력을 바꿔도 렌더가 예약되지 않아 재부착 문단 포커스 시 canvas에 머물러 편집 불가 — `_onFocus`에서 canvas 문단 포커스 진입 시 DOM 복귀 렌더 예약으로 수정 | ALL PASS (9항목, 서버 없으면 자동 기동) |
